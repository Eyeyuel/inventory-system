...
import {
  ...
  Stock,
  StockMovement,
} from '@inventory-system/entities';
import {
  PurchaseOrderStatusType,
  SalesOrderStatusType,
  StockMovementReasonsTypeForAdjust,
  StockMovementType,
} from '@inventory-system/types';
...
export class StockService {
  ...
  // stock.service.ts
  async findAll(filters: FindStocksDto, userId: string) {
    try {
      const {
        page = 1,
        limit = 10,
      ...
      const queryBuilder = this.dataSource
        ...
        .where('stock.user = :userId', { userId });
      ...
      if (search) {
        queryBuilder.andWhere('(product.name ILIKE :search OR location.name ILIKE :search)', {
          search: `%${search}%`,
        });
      }

      // Handle sorting on related fields (e.g., product.name)
      let orderBy: string;
      if (sortBy === SortableStockFields.PRODUCT_NAME) {
        orderBy = 'product.name';
      } else if (sortBy === SortableStockFields.LOCATION_NAME) {
        orderBy = 'location.name';
      } else {
        orderBy = `stock.${sortBy}`;
      }
      ...
      handleRpcException(error, 'Database error while fetching stock records');
    }
  }
  ...
  //       .getRepository(Stock)
  //       .find({ where, relations: ['location', 'product'] });
  //     if (stocks.length === 0) {
  ...
  async findAllStockMovements(filters: FindStockMovementsDto, userId: string) {
    try {
      const {
        page = 1,
        limit = 10,
      ...
      // Apply filters
      if (stockId) {
        queryBuilder.andWhere('stock.id = :stockId', { stockId });
      }
      if (productId) {
        queryBuilder.andWhere('product.id = :productId', { productId });
      }
      ...
      if (type) {
        queryBuilder.andWhere('sm.type = :type', { type });
      }
      if (referenceId) {
        queryBuilder.andWhere('sm.referenceId = :referenceId', { referenceId });
      }
      if (search) {
        queryBuilder.andWhere(
          '(sm.reason ILIKE :search OR sm.description ILIKE :search OR sm.referenceId ILIKE :search)',
          { search: `%${search}%` },
        );
      }
      if (fromDate) {
        queryBuilder.andWhere('DATE(sm.createdAt) >= :fromDate', { fromDate });
      }
      if (toDate) {
        queryBuilder.andWhere('DATE(sm.createdAt) <= :toDate', { toDate });
      }
      if (minQuantityChange !== undefined) {
        queryBuilder.andWhere('sm.quantityChange >= :minQuantityChange', { minQuantityChange });
      }
      if (maxQuantityChange !== undefined) {
        queryBuilder.andWhere('sm.quantityChange <= :maxQuantityChange', { maxQuantityChange });
      }

      const [data, total] = await queryBuilder
        .orderBy(`sm.${sortBy}`, sortOrder)
        .skip((page - 1) * limit)
        .take(limit)
        .getManyAndCount();
      ...
      handleRpcException(error, 'Database error while fetching stock movements');
    }
  }

  // async findAllStockMovements(
  //   filters: GetStockMovementsQueryDto,
  //   userId: string,
  ...
  //   const [entities, total] = await this.dataSource.getRepository(StockMovement).findAndCount({
  //     where,
  //     relations: ['stock', 'stock.location', 'stock.product'],
  ...
  async reciveStock(reciveStockDto: ReceiveStockDto, userId: string) {
    ...
    try {
      ...
      const location = await locationRepo.findOneBy({
        id: reciveStockDto.locationId,
        user: userId,
      });
      if (!location) {
        throw new RpcException({
      ...
      if (reciveStockDto.reasonCode === 'opening_stock' && !isNewStock) {
        throw new RpcException({
          statusCode: 400,
          message: 'Opening stock can only be set on the first movement for a product-location',
        });
      }
        ...
        if (reciveStockDto.purchaseOrderItemId) {
          ...
          if (poItem.quantityOrdered === poItem.quantityReceived) {
            throw new HttpException(
              {
                message:
                  'Purchase order item has already been fully received. Cannot receive more.',
              },
              400,
            );
          }
          ...
          const updatedPOI = purchaseOrderItemRepo.create({
            ...poItem,
            quantityReceived: poItem.quantityReceived + reciveStockDto.quantity,
          });
          ...
          if (mainPO) {
            ...
            const allItems = await purchaseOrderItemRepo.findBy({
              purchaseOrder: { id: mainPO.id },
            });
          ...
          throw new RpcException({
            statusCode: 400,
            message: 'purchaseOrderItemId is needed to update purchase order',
          });
        }
      }
      ...
      if (newQty < 0) {
        throw new BadRequestException(
          `Insufficient stock at ${stock.location?.name || 'location'}`,
        );
      }
      ...
      if (shipStockDto.reasonCode === 'sale') {
        if (shipStockDto.salesOrderItemId) {
          ...
          if (newQty > soItem.quantityOrdered) {
            throw new BadRequestException(
              `Quantity ${newQty} is more than ordered quantity ${soItem.quantityOrdered}.`,
            );
          }
          ...
          const updatedSOI = salesOrderItemRepo.create({
            ...soItem,
            quantityShipped: soItem.quantityShipped + shipStockDto.quantity,
          });
          ...
          if (mainSO) {
            ...
            const partiallyShipped = allItems.some(
              (item) => item.quantityOrdered > item.quantityShipped && item.quantityShipped > 0,
            );
            if (allShipped) {
          ...
          throw new RpcException({
            statusCode: 400,
            message: 'salesOrderItemId is needed to update sales order',
          });
        }
      }
  ...
  async transfer(transferStockDto: TransferStockDto, userId: string) {
    ...
    try {
      ...
      const fromLocation = await locationRepo.findOneBy({
        id: transferStockDto.fromLocationId,
        user: userId,
      });

      const toLocation = await locationRepo.findOneBy({
        id: transferStockDto.toLocationId,
        user: userId,
      });
      ...
      if (!fromLocation || !toLocation) {
        throw new RpcException({
          statusCode: 400,
          message: 'Verify that both fromLocation and toLocation belog to the user.',
        });
      }
