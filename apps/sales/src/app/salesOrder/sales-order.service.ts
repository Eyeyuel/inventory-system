...
export class SalesOrderService {
  ...
  async create(createSalesOrderDto: CreateSalesOrderDto, userId: string) {
    ...
    try {
      ...
      const openItemsFiltered = openItems.filter(item => item.quantityOrdered > item.quantityShipped);
      ...
      for (const item of createSalesOrderDto.items) {
        ...
        if (available < item.quantityOrdered) {
          ...
          throw new RpcException({
            statusCode: 400,
            message: `Insufficient available stock for product "${productName}" at location "${locationName}". ` +
