...
import {
  ...
  StockMovementReasonsTypeForReceive,
  StockMovementReasonsTypeForShip,
  StockMovementType,
...
export class TransferStockDto {
  ...
  @ApiProperty({
    description: 'Product UUID to transfer',
  ...
  @ApiProperty({
    description: 'Source location UUID',
...
export class AdjustStockDto {
  ...
  @ApiProperty({
    ...
    enum: StockMovementReasonsTypeForAdjust,
    example: 'damage',
  })
  ...
  @ApiPropertyOptional({
    ...
    example: 'Damaged items from warehouse shelf',
  })
  @IsOptional()
  @IsString()
  description?: string;
...
export class StockNestedDto {
  @ApiProperty({
    ...
    example: '7c740465-bd4c-4cc9-a273-31497166f268',
    format: 'uuid',
  })
  id!: string;
...
export class StockMovementSummaryDto {
  @ApiProperty({
    ...
    example: 'aa169ec1-ebc2-4ddf-86cb-e93bf89fde8b',
    format: 'uuid',
  })
  id!: string;
  ...
  @ApiProperty({
    ...
    example: 'receive OR ship OR adjust OR transfer',
    enum: StockMovementType,
  })
  type!: string;
  ...
  @ApiProperty({
    ...
    example: '2026-04-06T09:05:48.130Z',
    format: 'date-time',
  })
  createdAt!: Date;
...
export enum SortableStockMovementFields {
  TYPE = 'type',
...
export class FindStockMovementsDto {
  ...
  // Sorting
  @IsOptional()
  @IsEnum(SortableStockMovementFields)
  sortBy?: SortableStockMovementFields = SortableStockMovementFields.CREATED_AT;
  ...
  @IsOptional()
  @IsUUID()
  stockId?: string; // filter by specific stock record (product+location)

  @IsOptional()
  @IsUUID()
  productId?: string; // filter by product (via stock relation)

  @IsOptional()
  @IsUUID()
  locationId?: string; // filter by location (via stock relation)
  ...
  @IsOptional()
  @IsString()
  referenceId?: string; // exact match or partial? Use search?

  @IsOptional()
  @IsString()
  search?: string; // searches in reason, description, referenceId
